...
from kalman_innovation import InnovationCorrelationKalmanFilter
import numpy as np
import pickle
...
class NOAKalman:
    def __init__(self, F, B, H, Q, R, x0, P0, F2, hop_sec = 512 / 22050, steps = np.array([1, 1, 1, 2, 2, 1]).reshape((-1,2)), weights = np.array([1,1,2]), cost_metric = compute_cosine_distance, ref_start_time = 0, max_timewarp_factor = 3, onset_reduction_factor = 100):
        """
        ...
        Inputs:
            ...
            onset_reduction_factor: reduction factor for the measurement noise covariance when an onset is detected
        """
        ...
        self.onset_reduction_factor = onset_reduction_factor
        # feature info
        self.prev_feature = np.zeros(F2.shape[0])
    ...
    def detect_onset(self, f1):
        """ Check if the current feature is an onset """
        spectral_flux = f1 - self.prev_feature
        spectral_flux[spectral_flux < 0] = 0
        spectral_flux_norm = np.sum(spectral_flux, axis=0)
        self.prev_feature = f1
        if spectral_flux_norm > 0.5:
            return True
        else:
            return False
    ...
    def noa_update(self, f1):
        ...
        if self.detect_onset(f1):
            self.kalman_filter.R[0] = self.kalman_filter.R[0] / self.onset_reduction_factor
        
        costs = self.cost_metric(f1, self.F2)
        best_j = update_alignment_row_numba_norm(
            self.i, costs, self.D, self.B, self.dn, self.dm, self.weights, self.ref_length
        )
        self.noa_path.append([self.i, best_j])
        return best_j
    ...
    def get_noa_path(self):
        """
        Returns the path of the NOA alignment
        Outputs:
            path: warping path of shape (2, n_frames) where the first row is the indices of F1 and the second row is the indices of F2
        """
        path = np.array(self.noa_path, dtype=np.float32).T
        path[0, :] *= self.hop_sec
        path[1, :] *= self.hop_sec
        path[1, :] += self.ref_start_time
        return path
...
def alignNOAKalman(F1, F2, 
                   ...
                   return_path = True, onset_reduction_factor = 100):
    """
    ...
    noa_kalman = NOAKalman(F, B, H, Q, R, x0, P0, F2 = F2, hop_sec = 512 / 22050, steps = steps, weights = weights, cost_metric = cost_metric, ref_start_time = ref_start_time, onset_reduction_factor = onset_reduction_factor)
    noa_kalman.align(F1)
